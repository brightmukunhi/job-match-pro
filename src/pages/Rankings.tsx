...
export default function Rankings() {
  ...
  const runScoring = async () => {
      ...
      if (mode === "auto") {
        ...
        if (best) {
          ...
          results.push({
            ...
            departmentName: profile?.department_name || "Unknown",
          });
        }
      } else {
    ...
    const dbRows = results.map(r => ({
      ...
      position_profile_id: r.result.position_profile_id,
      qualification_points: r.result.qualification_points,
      skills_points: r.result.skills_points,
      certification_points: r.result.certification_points,
      attachment_points: r.result.attachment_points,
      total_score: r.result.total_score,
      skills_matched: r.result.skills_matched,
      certs_matched: r.result.certs_matched,
      must_haves_missing: r.result.must_haves_missing,
  ...
  const exportToExcel = () => {
    ...
    const data = rankings.map((r, i) => ({
      Rank: i + 1,
      Name: `${r.candidate.name} ${r.candidate.surname}`.trim(),
      Email: r.candidate.email,
      Phone: r.candidate.phone,
      Department: r.departmentName,
      Position: r.profileName,
      "Total (7)": r.result.total_score,
      "Qual (3)": r.result.qualification_points,
      "Skills (2)": r.result.skills_points,
      "Cert (1)": r.result.certification_points,
      "Attach (1)": r.result.attachment_points,
      "Skills Matched": r.result.skills_matched,
      "Must-Haves Missing": r.result.must_haves_missing,
      "Certs Matched": r.result.certs_matched,
      Notes: r.result.notes,
      File: r.candidate.file_name,
    }));
  ...
  // Group rankings by position for tab view
  const positionGroups = new Map<string, RankedCandidate[]>();
  for (const r of rankings) {
    const key = r.profileName;
    if (!positionGroups.has(key)) positionGroups.set(key, []);
    positionGroups.get(key)!.push(r);
  }
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Button variant="outline" onClick={exportToExcel} disabled={rankings.length === 0}>
              ...
              {mode === "position" && (
                <div className="space-y-2">
                  ...
                  <Select value={selectedProfile} onValueChange={setSelectedProfile}>
                    <SelectTrigger className="w-[280px]"><SelectValue placeholder="Select position" /></SelectTrigger>
                    <SelectContent>
    ...
    <Card className="mt-3">
      <div className="overflow-x-auto">
        <Table>
          ...
          <TableBody>
            {items.map((r, i) => (
              <TableRow key={`${r.candidate.id}-${r.result.position_profile_id}-${i}`}>
                ...
                <TableCell>
                  <Badge variant="outline" className="text-xs">{r.profileName}</Badge>
                </TableCell>
                <TableCell className="text-center">
                  <Badge variant={r.result.total_score >= 5 ? "default" : r.result.total_score >= 3 ? "secondary" : "outline"}>
                    {r.result.total_score}
                  </Badge>
                </TableCell>
                <TableCell className="text-center">{r.result.qualification_points}</TableCell>
                <TableCell className="text-center">{r.result.skills_points}</TableCell>
                <TableCell className="text-center">{r.result.certification_points}</TableCell>
                <TableCell className="text-center">{r.result.attachment_points}</TableCell>
                <TableCell className="text-xs max-w-[200px] truncate">{r.result.skills_matched}</TableCell>
